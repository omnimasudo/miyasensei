...
- Do not assume the OpenClaw agent's own model or API key will be reused by MiyaSensei.
- MiyaSensei classroom generation uses MiyaSensei server-side provider config.
- This skill must not rely on any request-time model or provider overrides.
- Only MiyaSensei server-side config files may control provider selection and defaults.
...
If no `accessCode` in config, ask the user how they want to use MiyaSensei:

1. **Use hosted MiyaSensei** (recommended for quick start) — Requires an access code from open.maic.chat. No local setup needed.
...
Use this when the user has not installed MiyaSensei yet or when you need to confirm which local checkout to use.
...
### 4. Start And Verify MiyaSensei

After the user has chosen a startup mode and configured keys, start MiyaSensei using the chosen method, then verify the service with `GET {url}/api/health`.
